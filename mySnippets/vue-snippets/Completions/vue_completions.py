...
def get_tag_to_attributes():
    ...
    global_attributes.extend([
        '@abort', '@autocomplete', '@autocompleteerror', '@auxclick', '@blur',
        '@cancel', '@canplay', '@canplaythrough', '@change', '@click',
        '@close', '@contextmenu', '@cuechange', '@dblclick', '@drag',
        '@dragend', '@dragenter', '@dragexit', '@dragleave', '@dragover',
        '@dragstart', '@drop', '@durationchange', '@emptied', '@ended',
        '@error', '@focus', '@input', '@invalid', '@keydown',
        '@keypress', '@keyup', '@load', '@loadeddata', '@loadedmetadata',
        '@loadstart', '@mousedown', '@mouseenter', '@mouseleave',
        '@mousemove', '@mouseout', '@mouseover', '@mouseup',
        '@mousewheel', '@pause', '@play', '@playing', '@progress',
        '@ratechange', '@reset', '@resize', '@scroll', '@seeked',
        '@seeking', '@select', '@show', '@sort', '@stalled', '@submit',
...
class HtmlTagCompletions(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        if not view.match_selector(locations[0], "text.html.vue - (source - source text.html)"
    ...
    def get_attribute_completions(self, view, pt, prefix):
        ...
        attri_completions += [('elseif\tv-else-if', 'v-else-if="$1"' + suffix)]
        attri_completions += [('else\tv-else', 'v-else' + suffix)]
        attri_completions += [('show\tv-show', 'v-show="$1"' + suffix)]
        attri_completions += [('mod\tv-model', 'v-model="$1"' + suffix)]
